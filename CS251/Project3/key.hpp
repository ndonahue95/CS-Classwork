...
Key& Key::operator+=(const Key& other) {
    ...
    for (int i = C-1; i >= 0; --i) {
       ...
       carry      = (t + other.m_digit[i] + carry) >= R;
        ...
        //std::cout << "Digit: " << i << "    Letter/Value: " << t << "    Carry: " << carry << '\n';
    }
	return *this;
}
...
Key& Key::operator-=(const Key& other) {
    ...
    for (int i = C-1; i >= 0; --i) {
        ...
        t = m_digit[i];
        m_digit[i] = ((t - other.m_digit[i] - carry) + R)  % R;
        carry      = (t - other.m_digit[i] - carry) < 0;
...
inline unsigned long long Key::getIntValue() const {
    ...
    std::string strFinal = stStream.str();


	return std::bitset<N>(strFinal).to_ullong();
